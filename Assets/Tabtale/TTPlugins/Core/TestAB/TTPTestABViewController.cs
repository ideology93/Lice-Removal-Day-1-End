...
namespace Tabtale.TTPlugins
{
    public class TTPTestABViewController : MonoBehaviour
    {
        ...
        private void ShowAllVariants(string[] variantNames)
        {
            ...
            {
                TTPTestAB.ApplyExperimentWithVariant(_selectedExperiment, variantName);
                SetActive(variantName);
            });
        }
        ...
        private void SetActive(string variantName)
        {
            Debug.Log("TTPTestABViewController::SetActive - " + variantName);
            
            foreach (var key in _buttons.Keys)
            {
                var img = _buttons[key].GetComponent<Image>();
                img.color = variantName == key ? Color.green : Color.white;
            }
        }
    }
}
